...
- `terraform plan -out=tfplan` 실행 전 반드시 다음 순서로 수행한다:
  ...
  3. **Checkov 보안 검사**: `checkov -d . --framework terraform --quiet` 실행
     ...
     - 검사 실패: FAILED 항목 목록 보고 후 각 항목에 대해 사용자에게 결정 요청 (human-in-the-loop)
       - **`checkov:skip` 추가는 Claude가 자동으로 하지 않는다** — 반드시 사용자가 직접 결정
       - 각 FAILED 항목별로: "수정" 또는 "skip 허용" 중 사용자 선택을 받은 후 진행
- `terraform plan -out=tfplan` 실행 후 반드시 다음 두 가지를 순서대로 수행한다:
  ...
  2. **예상 비용만** (실청구·실비용 조회는 하지 않음): `infracost breakdown --path tfplan --format table` 실행하여 예상 월 비용 보고
